"""Fine-tune TwHIN-BERT (Twitter/twhin-bert-base) on the tweet TEXT -> label, under the same
...
grouping by created_at keeps every user inside one fold. Outputs twhin_oof_probs.npy /
twhin_test_probs.npy. Run:  python finetune_camembert.py
"""
import os
...
np.save(os.path.join(HERE, "twhin_oof_probs.npy"), oof)
np.save(os.path.join(HERE, "twhin_test_probs.npy"), test_probs)
...
print(f"\n=== TwHIN-BERT fine-tuned (text only) ===")
print(f"OOF text-only: raw {best(oof)*100:.2f}%  | smoothed {best(smooth(oof))*100:.2f}%")
print("Saved twhin_oof_probs.npy / twhin_test_probs.npy for blending.")
